...
public:
    bool validTicTacToe(vector<string>& b)
    {
        ...
        if (wx && wo) return false;//不能都赢
        if (wx) return cx==co+1;//x赢了，必须比对方多一个子
        if (wo) return cx==co;//o赢了，双方子数应该相等
        return cx==co+1 || cx==co;//都没赢，
    }
private:
    ...
    bool win(vector<string>& b, char c)
    {
        ...
        for (int j=0; j<3; ++j) 
            if (b[0][j]==c && b[1][j]==c && b[2][j]==c) 
                return true;
        return (b[0][0]==c && b[1][1]==c && b[2][2]==c) || (b[2][0]==c && b[1][1]==c && b[0][2]==c);
    }
};
